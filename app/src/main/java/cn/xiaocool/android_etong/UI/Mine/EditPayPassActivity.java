 ...
 */
public class EditPayPassActivity extends Activity implements View.OnClickListener{
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case CommunalInterfaces.SEND_CODE:
                    ...
                    try {
                        ...
                        if (status.equals("success")) {
                            Toast.makeText(EditPayPassActivity.this, "发送验证码成功！", Toast.LENGTH_SHORT).show();
                            String data = jsonObject.getString("data");
                            JSONObject joCode = new JSONObject(data);
                            code = joCode.getString("code");
                            Log.e("code is ", code);
                        } else {
                            Toast.makeText(EditPayPassActivity.this, "验证码获取失败，请重试！", Toast.LENGTH_SHORT).show();
                        }
                ...
                case CommunalInterfaces.BTN_TOUCH:
                    ...
                    second =30;
                    break;
                case CommunalInterfaces.UpdatePayPass:
                    ...
                    try {
                        ...
                        if (status.equals("success")) {
                            //实力化缓存类
                            Toast.makeText(EditPayPassActivity.this, "修改支付密码！",Toast.LENGTH_SHORT).show();
                            finish();
                        }
                        else {
                            Toast.makeText(EditPayPassActivity.this, data,Toast.LENGTH_SHORT).show();
                        }
                    } catch (JSONException e) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_edit_paypass);
        context = this;
    ...
    private void sendCode() {
        ...
        if (phoneNumber.length() == 11) {
            ...
            Toast.makeText(EditPayPassActivity.this, "请输入正确的手机号码！", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void Next() {
        ...
        if (firstPassword.equals(secondPassword)&&!firstPassword.equals("")) {
            if ((phoneNum.length() == 11) && (verifyCode.length()) == 6) {
                new  MainRequest(context,handler).editpaypassword(phoneNum,verifyCode,firstPassword);
            } else {
                Toast.makeText(EditPayPassActivity.this, "请输入正确的验证码和手机号！！", Toast.LENGTH_SHORT).show();
            }
        }else
            Toast.makeText(EditPayPassActivity.this,"请检查输入的密码是否正确！",Toast.LENGTH_SHORT).show();
